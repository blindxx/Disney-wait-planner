...
/**
 * SH.3.3 — parsing/sanitization extracted from loadDayParks() below (mirrors
 * this file's own parseDayMetaRaw() extraction above exactly) so it can be
 * applied to either a plain canonical read (loadDayParks' own contract,
 * unchanged) OR the effective DURABLE raw value (readLatestDurableValue()) —
 * see loadEffectiveDurableDayParks() below, needed once dayParks becomes a
...
function loadDayParks(key: string): Record<string, string> {
  try {
    return parseDayParksRaw(localStorage.getItem(key));
  } catch {
    return {};
  }
}
...
/**
 * SH.3.3 — the dayParks domain's own authority-bearing read for THIS page's
 * own independent pull cycle, mirroring loadEffectiveDurableDayMeta() (above)
 * exactly: reuses parseDayParksRaw() against the effective DURABLE raw value
 * (canonical + any surviving local-edit fact published by Plans' own
 * commitLocalDomainRawSync-backed saveDayParks()) rather than a plain
 * canonical read, so Lightning's own sync/conflict decisions for this shared
 * domain see the same durable local authority Plans itself does.
 */
...
export default function LightningPage() {
  ...
  // Phase 8.8 — per-day park overrides (read-only UI; Plans page owns
  // writes). SH.3.3 — this page's own independent pull now also carries the
  // dayParks domain through cloud sync (a pass-through, like `plans` below),
  // so a device that only ever opens Lightning still keeps dayParks
  // hydrated from cloud.
  const dayParksKeyRef = useRef("dwp:default:dayParks");
  const [dayParks, setDayParks] = useState<Record<string, string>>({});
  // SH.3.3 — mirrors itemsRef/knownDaysRef/dayMetaRef: always holds the
  // latest dayParks state, used inside the async pull callback to avoid a
  // stale closure.
  ...
  // SH.3.3 — same fallback concept, applied to the newly synced dayParks
  // domain Lightning now carries through its own pull (a pass-through — see
  // dayMetaBaselineRef's own doc above; Lightning has no dayParks editing
  // UI, only Plans does).
  ...
  function buildPostFetchPullBaseline(
    ...
    return { items, days, dayMeta, dayParks, plansRaw, confirmed };
  }
  ...
  type UnusableDomain =
    ...
    | {
        ...
        // see its own detailed doc: a commit-time CAS supersession
  ...
  useEffect(() => {
    ...
    // SH.3.3 — read the domain's EFFECTIVE DURABLE value
    // (loadEffectiveDurableDayParks()), not a canonical-only read, mirroring
    // `loadedDayMeta` below for the same cross-tab-hydration-race reason.
    // Captured once so the SAME value seeds both the rendered state and
    // dayParksBaselineRef below — see that assignment's own doc.
    ...
    // SH.3.3 — same mount-time baseline fix as dayMeta above, applied to the
    // newly synced dayParks domain: seeded from the SAME `loadedDayParks`
    // read above, mirroring itemsBaselineRef/daysBaselineRef/
    // dayMetaBaselineRef exactly.
  ...
  useEffect(() => {
    ...
    void pullPlanner(pullCtx.profileId, pendingOpIds)
      .then(async (planner) => {
        ...
        // SH.2.2 — COMMIT-TIME AUTHORITY & RECONCILIATION RECOVERY. Mirrors
        // plans/page.tsx exactly, see its own detailed doc for the full
        // architecture: a winner selected here must remain PROVISIONAL
        // until the instant it is actually committed — confirmed authority
        // can advance (an already-in-flight push resolves and confirms a
        // newer revision) and local authority can advance (an ordinary user
        // edit lands) during the awaited gaps between this baseline check
        // and each domain's own commitLocalDomainRaw() write, and again
        // between commits. `supersededDomains` accumulates every
        // commit-time supersession this pull observes (either kind) so ONE
        ...
        function handlePullDeferral(unusable: UnusableDomain[]): void {
          ...
          // SH.2.1/SH.2.2 — see decideStaleResponseRecovery()'s own doc in
          // syncPayload.ts for the full, shared retry-eligibility rule.
        ...
        // SH.3.3 — dayParks domain: this page's own independent pull carries
        // it through cloud sync too (a pass-through, mirroring dayMeta above
        // — Lightning has no dayParks editing UI, only Plans does), gated on
        // `!daysWriteFailed` for the identical reason dayMeta's own gating
        // comment above gives. Pruned via the same pruneOrphanedDayRecord()
        // Plans' own local dayParks lifecycle already uses, so the "dayParks
        // entries only belong to valid durable day IDs" invariant holds
        // regardless of which page's pull last touched it.
        ...
        // reached the end of its commit sequence with one or more domains
        // having lost their own commitLocalDomainRaw() CAS to a genuine
        // local edit along the way. No-op when `supersededDomains` is
        // empty (the ordinary, no-conflict pull).
